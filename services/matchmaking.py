...
class MatchmakingService:
    ...
    async def find_partner(client: Client, user_id: int) -> Optional[int]:
        ...
        pref = await match_state.get_user_preference(user_id)
        candidates = await match_state.get_queue_candidates()
        
        for candidate in candidates:
            if candidate == user_id: continue
            
            cand_pref = await match_state.get_user_preference(candidate)
            if pref != 'Any' and cand_pref != 'Any' and pref != cand_pref:
                continue
            
            # ONE matching path
            success, _ = await distributed_state.atomic_claim_match(user_id, candidate)
            if success:
    ...
    @staticmethod
    async def request_rematch(user_id: int, partner_id: int) -> tuple[int, str]:
        """Handles rematch logic via distributed_state. Returns (success, code)."""
        from services.distributed_state import distributed_state
        return await distributed_state.atomic_rematch(user_id, partner_id)
