...
class UserService:
    ...
    async def report_user(reporter_id: int, reported_id: int, reason: str = "Excessive reports") -> bool:
        ...
        # Check report count
        row = await UserRepository.get_by_telegram_id(reported_id)
        reports_count = (row.get("reports") or 0) + 1
        ...
        await UserRepository.update(reported_id, reports=reports_count)
        return is_blocked
    ...
    async def check_milestones(user_id: int, challenge_type: str) -> Optional[Dict[str, Any]]:
        ...
        if challenge_type in milestones and val in milestones[challenge_type]:
            reward_xp = val // 5
            reward_coins = val // 10
