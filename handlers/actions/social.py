...
class SocialHandler:
    ...
    async def handle_user_appeal(client: Client, user_id: int) -> Dict[str, Any]:
        ...
        ... # Keep existing or use the one above, let's keep exact match below. Replace the method since it was the last one.
        return {
    ...
    async def handle_remove_friend(client: Client, user_id: int, friend_id: int) -> Dict[str, Any]:
        # Simple query directly if no repo method
