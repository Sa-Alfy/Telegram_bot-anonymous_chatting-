...
class MatchingHandler:
    ...
    async def handle_stop(client: Client, user_id: int) -> Dict[str, Any]:
        ...
        return {
            "text": user_summary,
            "reply_markup": end_menu(can_rematch=True),
            "partner_msg": {
    ...
    async def handle_next(client: Client, user_id: int) -> Dict[str, Any]:
        ...
        if stats:
            ...
            await update_user_ui(client, partner_id, partner_summary, end_menu(can_rematch=True))
        
        # Auto-search for a new partner
        success = await MatchmakingService.add_to_queue(user_id)
        if not success:
