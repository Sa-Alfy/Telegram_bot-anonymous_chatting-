...
class ReportRepository:
    ...
    async def create(reporter_id: int, reported_id: int, reason: str = None) -> int:
        ...
        query = "INSERT INTO reports_bans (reporter_id, reported_id, reason, timestamp) VALUES (?, ?, ?, ?)"
    ...
    async def update_status(report_id: int, status: str, admin_id: int) -> bool:
        ...
        query = "UPDATE reports_bans SET status = ?, admin_review_id = ? WHERE report_id = ?"
    ...
    async def create_appeal(user_id: int, reason: str) -> int:
        ...
        query = "INSERT INTO appeals (user_id, reason, timestamp) VALUES (?, ?, ?)"
