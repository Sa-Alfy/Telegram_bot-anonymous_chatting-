...
class FriendRepository:
    ...
    async def send_request(user_id: int, friend_id: int) -> bool:
        ...
        try:
            ...
            query = "SELECT status FROM friends WHERE (user_id = $1 AND friend_id = $2) OR (user_id = $3 AND friend_id = $4)"
            existing = await db.fetchone(query, (user_id, friend_id, friend_id, user_id))
            
            if existing:
                return False # Already friends or request pending
                
            query = "INSERT INTO friends (user_id, friend_id, status, created_at) VALUES ($1, $2, 'pending', $3)"
    ...
    async def accept_request(user_id: int, friend_id: int) -> bool:
        ...
        try:
            query = "UPDATE friends SET status = 'accepted' WHERE (user_id = $1 AND friend_id = $2) OR (user_id = $3 AND friend_id = $4)"
    ...
    async def get_friends_list(user_id: int) -> List[Dict[str, Any]]:
        ...
        query = """
            ...
            WHERE (f.user_id = $1 OR f.friend_id = $2) AND f.status = 'accepted' AND u.telegram_id != $3
        """
    ...
    async def is_friend(user_id: int, friend_id: int) -> bool:
        ...
        query = "SELECT 1 FROM friends WHERE ((user_id = $1 AND friend_id = $2) OR (user_id = $3 AND friend_id = $4)) AND status = 'accepted'"
    ...
    async def has_pending_request(user_id: int, friend_id: int) -> bool:
        ...
        query = "SELECT 1 FROM friends WHERE ((user_id = $1 AND friend_id = $2) OR (user_id = $3 AND friend_id = $4)) AND status = 'pending'"
        row = await db.fetchone(query, (user_id, friend_id, friend_id, user_id))
        return row is not None
    ...
    async def remove_friend(user_id: int, friend_id: int) -> bool:
        ...
        try:
            query = "DELETE FROM friends WHERE (user_id = $1 AND friend_id = $2) OR (user_id = $3 AND friend_id = $4)"
            await db.execute(query, (user_id, friend_id, friend_id, user_id))
            return True
        except Exception as e:
