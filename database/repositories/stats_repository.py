...
class StatsRepository:
    ...
    async def get_leaderboard(category: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        where_clause = ""
        if category == "vip":
            where_clause = "WHERE vip_status = 1"
        
        sort_field = order_by.get(category, "xp DESC")
        query = f"SELECT telegram_id, first_name, xp, level, vip_status FROM users {where_clause} ORDER BY {sort_field} LIMIT ?"
    ...
    async def get_top_event_participants(event_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        # This assumes seasonal_events is a JSON field in 'users'
        query = "SELECT telegram_id, first_name, json_extract(seasonal_events, '$.event_points') as points FROM users WHERE json_extract(seasonal_events, '$.current_event_id') = ? ORDER BY points DESC LIMIT ?"
