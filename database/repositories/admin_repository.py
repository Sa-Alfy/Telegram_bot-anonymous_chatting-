...
class AdminRepository:
    ...
    async def log_action(admin_id: int, action: str, target_user_id: int = None, details: str = None) -> int:
        ...
        query = "INSERT INTO audit_logs (admin_id, action, target_user_id, details, timestamp) VALUES (?, ?, ?, ?, ?)"
    ...
    async def get_banned_users() -> List[Dict[str, Any]]:
        ...
        query = "SELECT telegram_id, username, first_name, is_blocked FROM users WHERE is_blocked = 1"
    ...
    async def get_system_stats() -> Dict[str, Any]:
        ...
        row = await db.fetchone("SELECT COUNT(*) as count FROM sessions WHERE start_time > ?", (last_24h,))
